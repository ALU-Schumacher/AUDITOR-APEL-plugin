...
async def run(config, client):
    ...
    while True:
        ...
        try:
            start_time = await core.get_start_time(time_db_conn)
            ...
            summary_db = await core.create_summary_db(config, records_summary)
            grouped_summary_list = await core.group_summary_db(summary_db)
            ...
            post_summary = await core.send_payload(
                config, token, payload_summary
            )
            logging.debug(post_summary.status_code)

            begin_previous_month = await core.get_begin_previous_month(
                current_time
            )
            records_sync = await client.get_stopped_since(begin_previous_month)
            sync_db = await core.create_sync_db(config, records_sync)
            grouped_sync_list = await core.group_sync_db(sync_db)
            sync = await core.create_sync(grouped_sync_list)
            logging.debug(sync)
            signed_sync = await core.sign_msg(config, sync)
            logging.debug(signed_sync)
            encoded_sync = base64.b64encode(signed_sync).decode("utf-8")
            logging.debug(encoded_sync)
            payload_sync = await core.build_payload(encoded_sync)
            logging.debug(payload_sync)
            post_sync = await core.send_payload(config, token, payload_sync)
            logging.debug(post_sync.status_code)

            latest_report_time = datetime.now()
            await core.update_time_db(
